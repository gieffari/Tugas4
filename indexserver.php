...
// var_dump($input);die*();
$link = mysqli_connect('localhost', 'id8951566_davisdan', 'endrasumarno', 'id8951566_cacastie');
...
if ($params == 'data') {
	switch ($method) {
		case 'GET':
	    {
		    if (empty($id))
		    {
			    ...
			    // echo "select * from minuman ";break;
		    }
		    else
		    {
		         $sql = "select * from minuman where id_minuman='$id'";
		         // echo "select * from minuman where id='$id'";break;
		    }
	    }
	}
    ...
    if ($params == "create") {
	    ...
	    $harga=$data["harga"];
	    $stok=$data["stok"];
	    $querycek = "SELECT * FROM minuman WHERE minuman like '$nama'";
		$result=mysqli_query($link,$querycek);
		if (mysqli_num_rows($result) == 0)
		{
			$query = "INSERT INTO minuman (nama,harga,stok)	VALUES ('$nama','$harga','$stok')";
			
			mysqli_query($link,$query);
			$resp = array('status' => true, 'message' => "Minuman $nama telah ditambahkan");
		} else { 
			$resp = array('status' => false, 'message' => 'Nama minuman sudah terdaftar');
		}
    } elseif ($params == "update") {
	    $id_minuman=$data["id_minuman"];        
	    $nama=$data["nama"];
	    $harga=$data["harga"];
	    $stok=$data["stok"];
	    
	    $query = "UPDATE minuman 
	    	SET nama = '$nama',harga = '$harga',stok = '$stok'WHERE id_minuman ='$id_minuman'";
	    if (mysqli_query($link,$query)) {
			$resp = array('status' => true, 'message' => "Minuman $nama telah diperbarui");
	    } else {
